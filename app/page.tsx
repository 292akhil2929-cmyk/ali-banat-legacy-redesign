...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver((entries) => {
      const current = entries.find((entry) => entry.isIntersecting)?.target as HTMLElement | undefined;
      if (!current) return;
      setRailOnDark(current.classList.contains("hero") || current.classList.contains("dark-story") || current.classList.contains("blue-mission") || current.classList.contains("impact-section") || current.classList.contains("final-cta"));
  ...
  return (
    <main>
      ...
      <section className="intro section-paper chapter-one" id="story"><nav className={`chapter-rail ${railOnDark ? "is-on-dark" : "is-on-light"}`} aria-label="Story chapters">{chapterRailItems.map(([id, label], index) => <a className="chapter-rail-item" href={`#${id}`} key={id}><span className="chapter-rail-tick" /><span className="chapter-rail-label"><b>{String(index + 1).padStart(2, "0")}</b>{label}</span></a>)}</nav><div className="content-grid chapter-one-grid"><div className="chapter-one-lead"><p className="section-index">Chapter 01 — A life of success</p><h2>Who was <i>Ali Banat?</i></h2><p className="chapter-one-subtitle">Ali Banat, founder of MATW Project<br /><span>Ali Banat (1982–2018), founder of Muslims Around The World Project</span></p><figure className="chapter-one-image"><img src="/assets/ali-banat-child.jpg" alt="Ali Banat with a child during his humanitarian work" /><figcaption><span>Before the diagnosis</span><strong>A life that looked complete.</strong></figcaption></figure></div><div className="body-copy"><p>Before his diagnosis, Ali Banat was a successful businessman living in Sydney, Australia. He had worked hard to build a comfortable life and enjoyed many of the things commonly associated with achievement — businesses, luxury vehicles, designer clothing and material security.</p><p>From the outside, it looked like success.</p><p>But in 2015, Ali received news that would change the direction of his life: a diagnosis of an aggressive form of cancer, and the possibility that his time was limited. The value of everything around him changed. The question was no longer how much he could accumulate.</p><blockquote>“What can I send ahead for my Akhirah?”</blockquote><p>Ali understood that wealth was an Amanah. Time was an Amanah. Health was an Amanah. And whatever remained of his life was an opportunity to seek the pleasure of Allah.</p><p>He began giving away possessions that had once been important to him. He stepped away from the life he had known and focused his attention on people who had far less. He did not know how much time he had left — but he knew how he wanted to use it.</p></div></div></section>
